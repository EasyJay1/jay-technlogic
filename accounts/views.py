...
from .forms import StaffAddForm, StudentAddForm, ProfileUpdateForm, MessageForm, EmailValidationOnForgotPassword, RegistartionForm
...
def generate_pdf(request):
    ...
    try:
        ...
        return redirect('add_Client')  # Replace 'Client_form' with your actual URL name for the Client form
    Client = Client.objects.get(user=request.user)
    users_with_same_email = Client.objects.filter(email_p=Client.email_p).exclude(user=request.user)
    current_session = Session.objects.filter(is_current_session=True).first()
    current_semester = Semester.objects.filter(is_current_semester=True, session=current_session).first()
    all_notes = Client.objects.filter(user=request.user)
    if request.user.is_student:
        student = Student.objects.get(student=request.user)
        classes = Student.objects.get(student__pk=request.user.id)
        try:
            Client = Client.objects.get(student=classes)
        except:
...
def Clientview(request):
    Client = Client.objects.get(user=request.user)
    users_with_same_email = Client.objects.filter(email_p=Client.email_p).exclude(user=request.user)
    return render(request, 'accounts/404.html', {'Client': Client, 'users_with_same_email': users_with_same_email})

@login_required
def ClientAdd(request):
    existing_Client = Client.objects.filter(user=request.user).first()
    if existing_Client:
        messages.warning(request, 'You already have a Client associated with your account.')
    ...
    if request.method == 'POST':
        ...
        save_obj = Client(user=request.user, first_name=first_name, last_name=last_name,
    ...
    return render(request, 'accounts/Client_form.html', {'relationship_options': relationship_options})
